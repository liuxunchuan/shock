...
C   ROTATIONAL LEVELS OF THE X VIBRATIONAL GROUND STATE OF p-NH3
...
C     public EVOLpNH3
      ...
      contains
       ...
       SUBROUTINE pNH3_LVG(ABH,ABH2,ABHE,ABCO,TN0,ROP,VGRAD,
...
C INITIALIZE THE p-NH3 ROTATIONAL LEVEL POPULATION DENSITIES TO THEIR
C VALUES IN LTE AT THE NEUTRAL TEMPERATURE (ON FIRST CALL ONLY)

      IF(ICOUNT.EQ.1) CALL INITpNH3(ABCO,ROPNH3,TN0)
   
      CALL MATRCO(ABH, ABH2, ABHE, ABCO, TN0, TBB, ROP, WDIL,
     1            VGRAD,ROPNH3,EVLTOT)
      CALL EVOLpNH3 (EVLTOT,YTCO)
...
C2150 FORMAT(6D12.4)
      ...
      RETURN
      END SUBROUTINE pNH3_LVG
C
C      %%%%%%%%%%%%%%%%%
       SUBROUTINE INITpNH3(ABCO,ROPNH3,TN0)
C      %%%%%%%%%%%%%%%%%
...
C   NLEVCO:  NUMBER OF ROVIBRATIONAL LEVELS OF p-NH3 (X)

       PARAMETER (NLEVCO=24)
! The value of the PARAMETER NLEVCO must be the same as that of the corresponding 
! PARAMETER NpNH3_lev in evolution.f90, mhd_vode.f90 and outputs.f90
...
C   RATE COEFFICENTS ARE TABULATED
      ...
      COMMON /FCTpNH3/ COVJ0(NLEVCO),freq
      COMMON /pNH3/ GCO,ELHM,NJLEV
      
      COMMON /TRANSF_pNH3/QHCO,QHECO,QoH2CO,QpH2CO,EVLDI,AEITQ,TTAB,
     1               TTABH,TTABHE
...
C  ENERGIES, IN CM-1
     ...
     1   15.379600000,   16.169963295,  44.002600000,  44.793901871,
     2   55.145300000 ,  55.915793666,  103.628700000,  104.390366429, 
     3   114.743200000,  115.484863281,  138.564800000,  139.370004256,  
     4   183.035700000,  183.759646104,  194.112900000,  194.817864733, 
     5   204.475900000,  205.294232418, 237.859300000, 238.614923479, 
     6   282.143800000, 282.823318429, 293.174900000, 293.836635993/
...
C  COLLISIONAL RATE COEFFICIENTS FOR H + p-NH3 , HE + p-NH3 AND H2 + p-NH3 
...
C  RADIATIVE ELECTRIC DIPOLE TRANSITION PROBABILITIES OF p-NH3
C  and line frequency in GHz

       FICHIER = 'input/ApNH3.DAT              '
       OPEN(IREAD2, FILE = FICHIER, STATUS = 'OLD')
...
C   INITIAL POPULATIONS OF p-NH3
C   HYPOTHESIS 1: LTE AT INITIAL TEMPERATURE
   ...
   90    CONTINUE
         ...
         DO 100 LEV=1,NLEVCO
           IJ = NJLEV(LEV)
           COVJ0(LEV) = ABCO * COVJ0(LEV) / ECH / (1. + ROPNH3)
  100    CONTINUE

C   HYPOTHESIS 2: ENTIRE POPULATION IN GROUND STATE 


C        COVJ0(1) = ABCO / (1. + ROPNH3) 
...
C  THE LEVEL POPULATIONS OF p-NH3 
...
C      %%%%%%%%%%%%%%%%%%%%%%%%
       SUBROUTINE EVOLpNH3 (EVLTOT,YTCO)
C      %%%%%%%%%%%%%%%%%%%%%%%%
...
! in SUBROUTINE INITCO 
      ...
      COMMON /FCTpNH3/ COVJ(NLEVCO),freq,TAU,WWT,TVGRAD
      COMMON /pNH3/ GCO,ELHM,NJLEV
...
C  p-NH3 - HE (Machin & Roueff 2005, J. Phys. B, 38, 1519)
...
C  p-NH3 - p-H2 (Danby et al. 1988, MNRAS, 235, 229)
...
C---RADIATIVE COOLING BY p-NH3 (ERG CM-3 S-1)
c  GCO is the rate of energy loss by p-NH3 to the radiation field
...
C  VERIFY THAT THE SUM OF THE p-NH3 POPULATION DENSITIES (CM-3) 
C  IS EQUAL TO THE DENSITY OF THIS SPECIES
      verif = verif / (XNCO / ( 1. + ROPNH3))
      if (abs(verif-1.0d0) .gt. 1.0d-2) then
        print *, '  verif p-NH3:', verif, verif-1.0d0
      endif
