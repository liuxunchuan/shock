...
C   ROTATIONAL LEVELS OF THE X VIBRATIONAL GROUND STATE OF p-H2O
...
C     public EVOLpH2O
      ...
      contains
       ...
       SUBROUTINE pH2O_LVG(ABH,ABH2,ABHE,ABCO,TN0,ROP,VGRAD,
...
C INITIALIZE THE p-H2O ROTATIONAL LEVEL POPULATION DENSITIES TO THEIR
C VALUES IN LTE AT THE NEUTRAL TEMPERATURE (ON FIRST CALL ONLY)

      IF(ICOUNT.EQ.1) CALL INITpH2O(ABCO,ROPH2O,TN0)
   
      CALL MATRCO(ABH, ABH2, ABHE, ABCO, TN0, TBB, ROP, WDIL,
     1            VGRAD,ROPH2O,EVLTOT)
      CALL EVOLpH2O (EVLTOT,YTCO)
...
C2150 FORMAT(6D12.4)
      ...
      RETURN
      END SUBROUTINE pH2O_LVG
C
C      %%%%%%%%%%%%%%%%%
       SUBROUTINE INITpH2O(ABCO,ROPH2O,TN0)
C      %%%%%%%%%%%%%%%%%
...
C   NLEVCO:  NUMBER OF ROVIBRATIONAL LEVELS OF p-H2O (X)

       PARAMETER (NLEVCO=45)
! The value of the PARAMETER NLEVCO must be the same as that of the corresponding 
! PARAMETER NpH2O_lev in evolution.f90, mhd_vode.f90 and outputs.f90
...
C   RATE COEFFICENTS ARE TABULATED
      ...
      COMMON /FCTpH2O/ COVJ0(NLEVCO),freq
      COMMON /pH2O/ GCO,ELHM,NJLEV
      
      COMMON /TRANSF_pH2O/QHCO,QHECO,QoH2CO,QpH2CO,EVLDI,AEITQ,TTAB,
     1               TTABH,TTABHE
...
C  VALUES OF THE QUANTUM NUMBER, J   
     ...
     1       0, 1, 2, 2, 2, 3, 3, 4, 4, 3, 4, 5, 4, 5, 6, 4, 5, 6, 7,
     2       6, 5, 6, 7, 5, 8, 6, 7, 8, 6, 9, 7, 8, 6, 8, 7, 9, 10, 
     3       8, 7, 9, 8, 10, 11, 9, 7 /

C  ENERGIES, IN K
       DATA (ELHM(N), N=1,NLEVCO) /
     1     0.000000,   53.432892,  100.846649,  136.939120,  195.912630,
     2   204.710264,  296.826481,  319.489491,  396.385135,  410.373565, 
     3   454.343596,  469.948701,  552.272579,  598.841126,  642.707050,  
     4   702.327412,  725.109260,  781.132798,  843.826241,  867.270496, 
     5   877.832644,  951.836527, 1020.984284, 1067.694613, 1070.558777, 
     6  1090.294065, 1175.059616, 1270.302572, 1278.564607, 1323.998071, 
     7  1334.837897, 1414.213306, 1503.629930, 1510.966896, 1524.619513,
     8  1554.458454, 1603.588942, 1628.398582, 1749.853709, 1749.913425,
     9  1807.005436, 1860.394540, 1909.456706, 1929.265146, 2006.858477/
...
C  COLLISIONAL RATE COEFFICIENTS FOR H + p-H2O, HE + p-H2O AND H2 + p-H2O
...
C  RADIATIVE ELECTRIC DIPOLE TRANSITION PROBABILITIES OF p-H2O
C  and line frequency in GHz

       FICHIER = 'input/ApH2O.DAT              '
       OPEN(IREAD2, FILE = FICHIER, STATUS = 'OLD')
...
C   INITIAL POPULATIONS OF p-H2O
C   HYPOTHESIS 1: LTE AT INITIAL TEMPERATURE
   ...
   90    CONTINUE
         ...
         DO 100 LEV=1,NLEVCO
           IJ = NJLEV(LEV)
           COVJ0(LEV) = ABCO * COVJ0(LEV) / ECH / (1. + ROPH2O)
  100    CONTINUE

C   HYPOTHESIS 2: ENTIRE POPULATION IN GROUND STATE 


C        COVJ0(1) = ABCO / (1. + ROPH2O) 
...
C  THE LEVEL POPULATIONS OF p-H2O 
...
C      %%%%%%%%%%%%%%%%%%%%%%%%
       SUBROUTINE EVOLpH2O (EVLTOT,YTCO)
C      %%%%%%%%%%%%%%%%%%%%%%%%
...
! in SUBROUTINE INITCO 
      ...
      COMMON /FCTpH2O/ COVJ(NLEVCO),freq,TAU,WWT,TVGRAD
      COMMON /pH2O/ GCO,ELHM,NJLEV
...
C  p-H2O - HE (Green, Maluendes & McLean 1993, ApJS, 85, 181)
...
C  p-H2O - o-H2 (Faure et al. 2007, A&A, 472, 1029)
...
C  p-H2O - p-H2 (Faure et al. 2007, A&A, 472, 1029)
...
C---RADIATIVE COOLING BY p-H2O (ERG CM-3 S-1)
c  GCO is the rate of energy loss by p-H2O to the radiation field
...
C  VERIFY THAT THE SUM OF THE p-H2O POPULATION DENSITIES (CM-3) 
C  IS EQUAL TO THE DENSITY OF THIS SPECIES
      verif = verif / (XNCO / ( 1. + ROPH2O))
      if (abs(verif-1.0d0) .gt. 1.0d-2) then
        print *, '  verif p-H2O:', verif, verif-1.0d0
      endif
