...
C   LEVELS J.le.9 OF THE TORTIONAL GROUND STATE, nu = 0, OF E-type CH3OH
C   CALLED FROM SUBROUTINE DIFFUN
      ...
      contains
       ...
       SUBROUTINE Etype_LVG(ABH,ABH2,ABHE,ABCO,TN0,ROP,VGRAD,
...
C INITIALIZE THE E-type CH3OH ROTATIONAL LEVEL POPULATION DENSITIES TO THEIR
C VALUES IN LTE AT THE NEUTRAL TEMPERATURE (ON FIRST CALL ONLY)

      IF(ICOUNT.EQ.1) THEN
       CALL INIT_Etype(ABCO,RAE_CH3OH,TN0)
      ENDIF   

      CALL MATRCO(ABH, ABH2, ABHE, ABCO, TN0, TBB, ROP, WDIL,
     1            VGRAD,RAE_CH3OH,EVLTOT)
      CALL EVOL_Etype (EVLTOT,YTCO)
...
C2150 FORMAT(6D12.4)
      ...
      RETURN
      END SUBROUTINE Etype_LVG
C
C      %%%%%%%%%%%%%%%%%
       SUBROUTINE INIT_Etype(ABCO,RAE_CH3OH,TN0)
C      %%%%%%%%%%%%%%%%%
...
C   NLEVCO:  NUMBER OF ROTATIONAL LEVELS OF E-type CH3OH (nu = 0)

       PARAMETER (NLEVCO=256)
! The value of the PARAMETER NLEVCO must be the same as that of the corresponding 
! PARAMETER Netype_lev in evolution.f90, mhd_vode.f90 and outputs.f90
...
C   RATE COEFFICIENTS ARE TABULATED
      ...
      COMMON /FCTe_CH3OH/ COVJ0(NLEVCO),freq
      COMMON /Etype/ GCO,ELK,JLEV,KLEV
      
      COMMON /TRANSF_e_CH3OH/QHCO,QHECO,QoH2CO,QpH2CO,EVLDI,AEITQ,TTAB,
...
C  COLLISIONAL RATE COEFFICIENTS FOR HE + E-type CH3OH ET H2 + E-type  CH3OH 
...
c      CLOSE (IREAD2)
   ...
   18  CONTINUE
       ...
       FICHIER = 'input/QoH2_Etype_new.DAT              '
       OPEN(IREAD2, FILE = FICHIER, STATUS = 'OLD')
...
c           print *, NTRAN,(QoH2CO(IT,LEVU,LEVL),LEVU=1,NLEVCO)
   ...
   20  CONTINUE
       ...
       FICHIER = 'input/QpH2_Etype_new.DAT              '
       OPEN(IREAD2, FILE = FICHIER, STATUS = 'OLD')
...
C  LEVEL ENERGIES, IN CM-1, FOR E-TYPE, AND VALUES OF (J, K)

       FICHIER = 'input/Etype_new.DAT              '
       OPEN(IREAD2, FILE = FICHIER, STATUS = 'OLD')

       DO 25 ILEV=1,NLEVCO
        READ (IREAD2,1000) NLEV(ILEV),JLEV(ILEV),KLEV(ILEV),ELHM(ILEV)
 1000 FORMAT(I4, 3X, I5, I4, F15.0)
...
C  ELECTRIC DIPOLE TRANSITION PROBABILITIES FOR E-TYPE CH3OH
C  and line frequencies in GHz

       FICHIER = 'input/A_Etype_newer.DAT              '
       OPEN(IREAD2, FILE = FICHIER, STATUS = 'OLD')

        READ (IREAD2,*) NTRTOT
         IF(NTRTOT.GT.MLEVCO) THEN
          WRITE(6,2000)
 2000 FORMAT('CHECK DIMENSION MLEVCO OF THE E-TYPE TRANSITION 
...
c    2                  Eupcm, EupK
c       freq(LEVU,LEVL) = 2.9979D1 * (ELHM(LEVU) - ELHM(LEVL))/1.4388D0
   30  CONTINUE
      CLOSE (IREAD2)
...
c          print *, J, I, EVLDI(J,I), freq(J,I)
   40    CONTINUE
   45  CONTINUE

C*********************************************************************

C   INITIAL LEVEL POPULATIONS OF E-type CH3OH
C   HYPOTHESIS 1: LTE AT INITIAL TEMPERATURE
   ...
   90    CONTINUE
         ...
         DO 100 LEV=1,NLEVCO
           IJ = NJLEV(LEV)
           COVJ0(LEV) = ABCO * COVJ0(LEV) / ECH / (1. + RAE_CH3OH)
  100    CONTINUE

C   HYPOTHESIS 2: ENTIRE POPULATION IN GROUND LEVEL 


C        COVJ0(1) = ABCO / (1. + RAE_CH3OH) 
...
C  THE LEVEL POPULATIONS OF E-type CH3OH 
...
C      %%%%%%%%%%%%%%%%%%%%%%%%
       SUBROUTINE EVOL_Etype (EVLTOT,YTCO)
C      %%%%%%%%%%%%%%%%%%%%%%%%
...
! in SUBROUTINE INITCO 
      ...
      COMMON /FCTe_CH3OH/ COVJ(NLEVCO),freq,TAU,WWT,TVGRAD
      COMMON /Etype/ GCO,ELK,JLEV,KLEV
...
C  E-type - He (Rabli & Flower 2010, MNRAS, 403, 2033; 2011, MNRAS, 411, 2093)
...
C  E-type - o-H2 (Rabli & Flower 2010, MNRAS, 406, 95)
...
C  E-type - p-H2 (Rabli & Flower 2010, MNRAS, 406, 95)
...
c  to the density of E-type CH3OH
...
C---RADIATIVE COOLING BY E-type CH3OH (ERG CM-3 S-1)
c  GCO is the rate of energy loss by E-type CH3OH to the radiation field
...
C  VERIFY THAT THE SUM OF THE E-type CH3OH POPULATION DENSITIES (CM-3) 
C  IS EQUAL TO THE DENSITY OF THIS SPECIES
      verif = verif / (XNCO / ( 1. + RAE_CH3OH))
      if (abs(verif-1.0d0) .gt. 1.0d-2) then
        print *, '  verif E-type CH3OH:', verif, verif-1.0d0
      endif
